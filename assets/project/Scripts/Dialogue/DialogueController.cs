...
/// </summary>
public class DialogueController : DialogueViewBase
{
    ...
    [SerializeField] private RectTransform dialogueBubble = null;
    [SerializeField] private Image pointerRight = null;
    [SerializeField] private Image pointerLeft = null;

    private Vector2 pos0;

    private const string MAIN_CHARACTER = "Toto";
    void Awake()
    {
    ...
    void Start()
    {
        ...
        runner.onOptions.AddListener(() => SetSpriteOnOptions());
    ...
    public override void RunLine(LocalizedLine dialogueLine, System.Action onDialogueLineFinished)
    {
        ...
        {
            Debug.Log("HighlightSprite method invoked");
            Debug.Log("actors[actorName].actorImage = " + actors[actorName].actorImage);
            HighlightSprite(actors[actorName].actorImage);

            // pop effect
            var actor = actors[actorName].actorImage;
            float YPos0 = actor.transform.position.y;
            Sequence s = DOTween.Sequence();
            s.Append(actor.transform.DOMoveY(YPos0 + 15, 0.15f))
             .Append(actor.transform.DOMoveY(YPos0, 0.15f));

            SetDialoguePop();

            if (actorName == MAIN_CHARACTER)
            {
                dialogueBubble.anchoredPosition = new Vector2(30, pos0.y);
                pointerLeft.gameObject.SetActive(true);
            ...
            else
            {
                ...
                pointerRight.gameObject.SetActive(true);
            }
    ...
    IEnumerator HighlightSpriteCoroutine(Image highlightedSprite)
    {
        ...
        while (t < 1f)
        {
            ...
            foreach (var actor in actors)
            {
                ...
                var spr = actor.Value.actorImage;
                ...
                { // set back to normal
                    ...
                    spr.color = Color.Lerp(spr.color, defaultTint, 0.5f);
                }
                else
                { // a little bit bigger / brighter
                    ...
                    spr.color = Color.Lerp(spr.color, highlightTint, 0.5f);
    ...
    Image SetSpriteActual(string spriteName, Vector2 position)
    {
        ...
        newSpriteObject.rectTransform.anchorMax = new Vector2(0.5f, 0);
        newSpriteObject.rectTransform.anchorMin = new Vector2(0.5f, 0);
        newSpriteObject.rectTransform.pivot = new Vector2(0.5f, 0);
        newSpriteObject.rectTransform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
        // newSpriteObject.rectTransform.anchoredPosition = Vector2.Scale(position, screenSize);
        newSpriteObject.rectTransform.anchoredPosition = position;
    ...
    public void SetFloatingDialogue()
    {
        dialogueBubble.DOLocalMoveY(pos0.y - 5, 0.4f)
            .SetEase(Ease.InOutQuad)
            .From(pos0.y)
            .SetLoops(-1, LoopType.Yoyo);
    }

    public void SetDialoguePop()
    {
        Vector3 scale0 = dialogueBubble.localScale;
        Sequence s = DOTween.Sequence();
        s.Append(dialogueBubble.DOScale(scale0 + new Vector3(0.05f, 0.05f, 0.05f), 0.15f))
         .Append(dialogueBubble.DOScale(scale0, 0.05f));
    }
    public void SetSpriteOnOptions()
    {
        dialogueBubble.anchoredPosition = new Vector2(0, pos0.y);
        pointerLeft.gameObject.SetActive(false);
        pointerRight.gameObject.SetActive(false);
        HighlightSprite(actors[MAIN_CHARACTER].actorImage);
    }
